...
// ⚠️ 명세서엔 3개 필드뿐이었지만, 실제 백엔드가 PATCH여도 안 보낸 필드를 지워버려서
// projectName/description도 같이 보내야 기존 값이 안 사라짐 (실제 동작 기준으로 수정)
export interface UpdateStacksPayload {
  projectName: string;
  description: string;
  fieldIds: string[];
  stackIds: string[];
...
export interface UpdateProjectPayload {
  projectId: string;
  data: UpdateStacksPayload;
}

// PATCH /projects/{id}/stacks 와 POST /projects/structures 둘 다 202로
// {success, data:{projectId, status:"PENDING"}, error} 모양을 그대로 돌려줌 (명세서 기준)
export interface StructureStatusResponse {
  success: boolean;
  data: {
    projectId: string;
    status: string;
